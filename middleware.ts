...
export async function middleware(request: NextRequest) {
  ...
  if (host.endsWith('bonus-turboplay.bet')) {
  ...
  if (host.endsWith('gaminyalanyokt1.netlify.app')) {
    ...
    const targetUrl = new URL(`https://bonus-turboplay.bet${pathname}${search}`);
  ...
  //    - без user-agent или бот → редирект на yalanyokgaming.netlify.app
  //    - реальный пользователь → редирект на bonus-turboplay.bet
  //    - трафик из Meta также считаем реальным пользователем и ведём на bonus-turboplay.bet
  if (!host.endsWith('bonus-turboplay.bet')) {
    const headers: Record<string, string> = {};
    request.headers.forEach((value, key) => {
      headers[key] = value;
